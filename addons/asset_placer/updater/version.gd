...
func _init(version: String) -> void:
	...
	if result:
		...
		var _identifier: String = result.get_string("prerelease") if result.get_string("prerelease") != "" else ""
		self.identifier = _parse_identifier(_identifier)


func _to_string() -> String:
	var main: String = "%s.%s.%s" % [major, minor, patch]
...
func compare_prerelease(id1: Identifier, id2: Identifier) -> int:
	...
	if id1.track == id2.track:
		var diff: int = id1.number - id2.number;
		return clampi(diff, -1, 1)
	else:
		var track_diff: int = (id1.track - id2.track)
...
static func _parse_identifier(text: String) -> Identifier:
	...
	var track_str: String = match.get_string(1).to_lower()
	var number_str: String = match.get_string(2)
