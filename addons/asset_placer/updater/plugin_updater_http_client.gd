...
func client_get(url: String) -> void:
	AssetPlacerAsync.instance.enqueue(func():
		var response: PackedByteArray = _client_get(url)
...
func _client_get(url: String) -> PackedByteArray:
	var parts: PackedStringArray = url.split("/", false, 2)
	var host: String = parts[0] + "//" + parts[1]
	...
	if _client.get_response_code() in [301, 302, 308]:
		var location: String = _client.get_response_headers_as_dictionary()["Location"]
		_client.close()
		await_disconection()
		return _client_get(location)

	var response: PackedByteArray = _do_response_body_polling()
	_client.close()
	return response
...
func _do_connection_polling() -> void:
	while _is_connecting():
		_client.poll()

func _do_response_polling() -> void:
