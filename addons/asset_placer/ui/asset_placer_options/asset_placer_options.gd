...
@onready var min_scale: SpinBox = %MinScale
@onready var max_scale: SpinBox = %MaxScale
...
func _ready() -> void:
	...
	min_scale.value_changed.connect(presenter.set_min_scale)
	max_scale.value_changed.connect(presenter.set_max_scale)
...
func set_options(options: AssetPlacerOptions) -> void:
	...
	min_scale.set_value_no_signal(options.min_scale)
	max_scale.set_value_no_signal(options.max_scale)
